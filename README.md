...
* ```utils/```: You may have an additional directory ```utils/``` containing necessary scipts and files to run your model. Please be aware that we expect ```srmodel()``` to return your method with correct configuration, checkpoint etc. **without** input arguments.
